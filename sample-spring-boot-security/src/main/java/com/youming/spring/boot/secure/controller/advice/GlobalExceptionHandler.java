package com.youming.spring.boot.secure.controller.advice;
...
public class GlobalExceptionHandler {
	...
	/**
	 * spring security不允许访问
	 * */
	@ExceptionHandler({ AccessDeniedException.class })
	public void errorAccessDeniedHandler(HttpServletRequest request, HttpServletResponse response, Exception ex) {
		//ex.printStackTrace();
		logger.info("errorAccessDeniedHandler:" + ex.getMessage());
		response.setHeader("Content-type", "text/plain;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
