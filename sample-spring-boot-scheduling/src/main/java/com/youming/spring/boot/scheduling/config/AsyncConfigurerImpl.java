...
/**
 * Spring AsyncConfigurer调用的线程池配置
 * 如果当前线程数少于CorePoolSize，那么即使线程都是闲置的，也必须新建线程来处理任务
 ...
 * */
@Configuration
@EnableAsync // 异步任务@Async注解需要
public class AsyncConfigurerImpl implements AsyncConfigurer {

	private static final Logger logger = LoggerFactory.getLogger(AsyncConfigurerImpl.class);
	...
	public Executor getAsyncExecutor() {
		...
		threadPoolTaskExecutor.setRejectedExecutionHandler(new RejectedExecutionHandler() {
			...
			public void rejectedExecution(Runnable runnable, ThreadPoolExecutor threadPoolExecutor) {
				...
				logger.error("线程池超载异常;当前线程数:" + threadPoolExecutor.getActiveCount() + ",未处理任务数:" + threadPoolExecutor.getTaskCount());
			}
		});
